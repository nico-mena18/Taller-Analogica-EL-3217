...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figs/DS0002}
...
\begin{enumerate}[resume, label=\arabic*.]
    ...
    Observations: \uline{Se observa una señal cuadrada que cambia su valor (de alto a bajo) y el ancho de pulso aumenta al variar el potenciómetro.}
...
\centering
\includegraphics[width=\textwidth]{figs/DS0003}
...
\begin{minipage}{0.6\textwidth}
\centering
\includegraphics[width=\textwidth]{figs/DS0004}
...
\begin{enumerate}[resume, label=\arabic*.]
    ...
    Observations: \uline{Se observa como la curva de transferencia se satura postivamente al variar el potenciómetro. En la señal de la curva inversa se observa como se satura al aumentar el voltaje de referencia.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figs/DS0005}
...
\begin{enumerate}[resume, label=\arabic*.]
    ...
    Observations: \uline{En la curva de transferencia de este circuito se observa una "caja", ya que el circuito posee histéresis, ya que es un circuito retroalimentado.}

\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figs/DS0006}
...
\begin{enumerate}[label=\arabic*.]
	...
	A medida que se modifica el voltaje de umbral del comparador la curva de transferencia modifica el punto de conmutación del comparador. Haciendo que la forma de onda se desplace de manera horizontal. 
    \vspace{0.25cm}
    ...
    \vspace{0.5cm}
    Con $V_{ref} = 0\text{ V}$ el comparador conmuta cada vez que la señal de entrada cruza por $0 \text{V}$. Variar el offset de CD desplaza la señal verticalmente, por lo que los cruces por cero se desplazan a nivel temporal. Si la variación del offset no es mucha entonces solo afecta el ciclo de trabajo que se observa a la salida. En el caso que $V_{offset}$ sea mayor a $V_{p}$ entonces la tensión de salida sería constante con un valor de $V_{sat}$, ya que no existiría cruce por $0$.

    

    \item Would a sinusoidal input to the comparators produce the same transfer curve as a triangle waveform? Explain.\\
    
    Sí, ya que la función de transferencia es la salida $V_{out}$ en función de $V_{in}$, sin considerar la variable temporal. Si se tiene una onda triangular y otra sinusoidal, y tienen las mismas amplitudes, entonces la función de transferencia sería igual, considerando que tengan la misma referencia.

    \vspace{0.5cm}

    \item Summarize the important differences between a comparator and Schmitt trigger.\\
	El comparador disparador de Schmitt posee retroalimentación negativa lo que permite comparar valores específicos positivos y negativos, llamados puntos de disparo, que permiten comparar la señal en rangos positivos y negativos. Esto es de gran utilidad ante pequeñas variaciones como el ruido eléctrico en donde se quiere comparar una señal que posee oscilaciones y evitar de esta forma conmutaciones erróneas. En el caso del comparador normal, este compara un valor fijo por lo que es propenso a realizar conmutaciones erróneas ante pequeñas oscilaciones.
    ...
    Ya que el ruido es de $100\, mV_{pp}$ entonces los puntos de disparo deben ser $\pm 50\, mVp$.
    Si se siguen las ecuaciones de los puntos de disparo:
    \begin{equation}
    	V_{trigger} = \frac{R_i}{R_i + R_f} (\pm V_{sat})
    \end{equation}
    
    \begin{align*}
    	500\text{ mV} = \frac{R_i}{R_i + 100\text{ k}\Omega}(15\text{ V})
    \end{align*}
    Se obtiene que la resistencia mínima del potenciómetro debe ser:
    $R_i \approx 386 \Omega$
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{figs/DS0007}
	\caption{Plot 22-6 --- Schmitt trigger waveform as an oscillator.}
	\label{plot:22-6}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{figs/DS0008}
	\caption{Plot 22-6 --- Schmitt trigger transfer curve as an oscillator.}
	\label{plot:22-7}
\end{figure}

Observatios: \uline{Se puede obtener una onda senoidal cuando se ajusta la resistencia a $0\,\Omega$ o muy baja, con eso el slew rate se lleva al límite y no se pueden generar los cambios bruscos de una onda cuadrada y con eso se convierte en un filtro pasa-bajas. Por otro lado, en la curva de transferencia se observa una línea diagonal ya que el voltaje de entrada y el de referencia son exactamente iguales}
